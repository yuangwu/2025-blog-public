...
export default function BlurredBubblesBackground({
  ...
  // 气泡最大半径 (px)
  bottomBandStart = 0.8,
  // 垂直方向底部区域起始比例（0.8 表示从 80% 高度开始）
  speed = 0.12,
  // 基础移动速度系数
  noiseScale = 0.0008,
  // 空间噪声缩放（值越小，流场变化越平缓）
  noiseTimeScale = 0.00015,
  // 时间噪声缩放（值越小，随时间的流动变化越慢）
  targetFps = 6,
  // 目标帧率（降低性能消耗）
  debugFps = false,
  // 是否在控制台打印实际 FPS
  ...
  // 移动端首次启动延迟（避免页面加载时卡顿）
  regenerateKey = 0
  // 外部传入的 key，改变时强制重新生成所有气泡
}) {
  const ref = useRef<HTMLCanvasElement>(null) 
  // canvas 引用
  const noise = useRef(makeNoise2D())
  // 创建并保存一个二维噪声生成器，避免重复初始化
